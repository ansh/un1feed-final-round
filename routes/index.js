...
const { fetchHomeTimelineOnDemand } = require("../controllers/twitter");
var router = express.Router();

/* GET home page. */
router.get("/timeline", fetchHomeTimelineOnDemand);
...
router.get("/twitter", async function (req, res, next) {
  ...
  const creds = {
    ...
    consumer_api_secret_key:
      "q0iIRsuZmcP1uMcul8GDizhAz4g4EzcEI4fCU5tSU4Ys3TEeAx",
    bearer_token:
  ...
  const twitterClient = await client
    .get("account/verify_credentials")
    .catch(console.error);

  res.status(200).json({ name: twitterClient.name });
});
